...
public function measure(int bucketOne, int bucketTwo, int goal, string startBucket) returns TwoBucketSolution|error {
    [Bucket, Bucket] [small, big] = createBucket(bucketOne, bucketTwo);
    [Bucket, Bucket] [goalBucket, otherBucket] = 
        (small.name == startBucket && small.size >= goal) ? [small, big] : [big, small];
    Bucket strtBucket = small.name == startBucket ? small : big;
    strtBucket.fill();
    int moves = 1;
    io:println("goalBucket: ", goalBucket);
    io:println("otherBucket: ", otherBucket);
    ...
    while !(goalBucket.reachTarget(goal)) {
        ...
        io:println(`goalBucket: ${goalBucket.name} has state: ${goalBucket}`);
        io:println(`otherBucket: ${otherBucket.name} has state: ${otherBucket}`);
...
public function createBucket(int bucketOne, int bucketTwo) returns [Bucket, Bucket] {
