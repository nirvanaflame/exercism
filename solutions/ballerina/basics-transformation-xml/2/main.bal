...
function parseXml(xml fuelEvents) returns FillUpEntry[] {
    ...
    FillUpEntry[] typedEntries = from var fuelEvent in fuelEvents/<s:FuelEvent>
    select {
        employeeId: checkpanic  int:fromString(fuelEvent?.employeeId ?: "0"),
        odometerReading: checkpanic int:fromString((fuelEvent/<s:odometerReading>).data()),
        gallons: checkpanic decimal:fromString((fuelEvent/<s:gallons>).data()),
        gasPrice: checkpanic decimal:fromString((fuelEvent/<s:gasPrice>).data())
    };

    io:println(typedEntries);
